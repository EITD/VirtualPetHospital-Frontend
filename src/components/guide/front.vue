<template>
  ...
  <div class="all">
    <div class="panel">
      <div class="title">前台大厅</div>
      <div class="logout" @click="back">退出</div>
      <!-- <div class="identity">
      <div class="img"></div>
      <div class="name">兽医</div>
    </div> -->
      <div class="operation" v-if="equipmentName !== null">
        <div class="header">{{ equipmentName }}</div>
        <div class="item">
          <a-button
            style="margin: 10px"
            @click="
      ...
      <div ref="model3D" class="model-3d" id="model">
        <div class="modelPanel" v-if="showPanel === true">
          <div class="cancel">
            <img src="/img/cancel.png" alt="" @click="showPanel = false" />
          </div>
          <div class="panel1" v-if="panelId === 1">
            <h2 style="color: white">功能说明</h2>
            <a-typography-paragraph
              :ellipsis="{ rows: 14, expandable: false }"
              style="color: white; font-size: 1.3rem"
            >
              {{ text.get(equipmentName).funcDes }}
            </a-typography-paragraph>
          </div>
          <div class="panel1" v-else-if="panelId === 2">
            ...
            <a-typography-paragraph
              ...
              style="color: white; font-size: 1.3rem"
            >
              {{ text.get(equipmentName).operating }}
            </a-typography-paragraph>
          </div>
          <div class="panel3" v-else-if="panelId === 3">
            <h2 style="color: white">演示动画</h2>
            <video
              :src="text.get(equipmentName).video"
              controls
...
async function init3D() {
  ...
  async function onMouseClick(event) {
    ...
    // }
    let p = intersects[0]?.object.name;
    console.log(p);
    if (map.has(p)) {
    ...
    if (pointControls) {
      if (pointControls.isLocked) {
        ...
        if (moveForward || moveBackward)
          velocity.z -= direction.z * 200.0 * delta; // 可控制移动的速度
        ...
        // if (pointControls.getObject().position.x < -90 && pointControls.getObject().position.x > -120 && pointControls.getObject().position.z < 14 ) {
        ...
        // }
        console.log(
          pointControls.getObject().position.x,
          pointControls.getObject().position.z
        );
      } else {
      }
    }
