    ...
    <div class="modelPanel" v-if="showPanel === true">
      ...
      <div class="panel1" v-if="panelId === 1">
       ...
       <div class="panel3" v-else-if="panelId === 3">
        ...
        <video  :src="text.get(equipmentName).video" controls style="width:700px;height:400px"></video>
      </div>
    </div>
   </div>
   </div>
...
const map = new Map([
  ['OP_Mitte_Mitte_OP_Tex4_0001', '手术操控台'], 
  ['Archmodels70_064', 'CT'], 
  ['Tracy', 85]
])
const roomMap = new Map([
  ["defaultMaterial004", "front"],
]);
...
async function init3D() {
  ...
  async function onMouseClick(event) {
    ...
    // }
    let p = intersects[0]?.object.name.slice(0,-3)
    console.log(p)
      if(map.has(p)){
        equipmentName.value = map.get(p)
